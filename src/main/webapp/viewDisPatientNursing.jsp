...
//	페이징 작업
	...
	for (int subDay=0; subDay<=diffDay; subDay++) {
		...
		vitalSignList = service.selectVitalSignList_Menu(patientIdx, subDay + dToDay);
		injectionList = service.selectInjectionList_Menu(patientIdx, subDay + dToDay);
		nursingCommentList = service.selectNursingCommentList_Menu(patientIdx, subDay + dToDay);
...
//	request 영역에 저장된 데이터를 가지고 브라우저에 글을 출력하는 페이지로 넘겨준다.
	pageContext.forward("viewDisPatientNursingAfter.jsp");
